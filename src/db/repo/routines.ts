...
import type { Routine, RoutineDetail, RoutineExerciseDetail, RoutineSectionDetail, RoutineSet } from '../types';
...
export function listRoutines(db: AppDatabase): Routine[] {
  return db
    .all<{ _id: number; name: string; notes: string | null }>('SELECT * FROM Routine ORDER BY name COLLATE NOCASE ASC')
    .map((r) => ({ id: r._id, name: r.name, notes: r.notes ?? '' }));
}

export function getRoutine(db: AppDatabase, id: number): RoutineDetail | undefined {
  const r = db.get<{ _id: number; name: string; notes: string | null }>('SELECT * FROM Routine WHERE _id = ?', [id]);
...
export function listSectionExercises(db: AppDatabase, sectionId: number): RoutineExerciseDetail[] {
  ...
  return db
    .all<{ _id: number; routine_section_id: number; exercise_id: number; sort_order: number; populate_sets_type: number }>(
...
export function updateRoutine(db: AppDatabase, id: number, patch: { name?: string; notes?: string }): void {
  db.mutate(() => {
    if (patch.name !== undefined) db.run('UPDATE Routine SET name = ? WHERE _id = ?', [patch.name.trim(), id]);
...
export function deleteRoutine(db: AppDatabase, id: number): void {
  db.mutate(() => {
    for (const s of db.all<{ _id: number }>('SELECT _id FROM RoutineSection WHERE routine_id = ?', [id])) deleteSection(db, s._id);
  ...
  return db.mutate(() => {
    ...
    for (const section of src.sections) {
      ...
      for (const g of listRoutineSectionGroups(db, section.id)) {
        createGroup(db, { date: '', routineSectionId: newSectionId, name: g.name, colour: g.colour, exerciseIds: g.exerciseIds });
      }
    }
    return newId;
  });
...
export function addSection(db: AppDatabase, routineId: number, name: string): number {
  return db.mutate(() => {
    const sort = Number(db.scalar('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM RoutineSection WHERE routine_id = ?', [routineId]));
    db.run('INSERT INTO RoutineSection (routine_id, name, sort_order) VALUES (?, ?, ?)', [routineId, name.trim(), sort]);
    return Number(db.scalar('SELECT last_insert_rowid()'));
  });
}
...
export function reorderSections(db: AppDatabase, orderedSectionIds: number[]): void {
  db.mutate(() => {
    orderedSectionIds.forEach((id, i) => db.run('UPDATE RoutineSection SET sort_order = ? WHERE _id = ?', [i + 1, id]));
  });
}
...
export function addSectionExercise(db: AppDatabase, sectionId: number, exerciseId: number): number {
  return db.mutate(() => {
    const sort = Number(
      db.scalar('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM RoutineSectionExercise WHERE routine_section_id = ?', [sectionId]),
    );
    db.run('INSERT INTO RoutineSectionExercise (routine_section_id, exercise_id, sort_order, populate_sets_type) VALUES (?, ?, ?, 0)', [
      sectionId,
      exerciseId,
      sort,
    ]);
...
export function removeSectionExercise(db: AppDatabase, routineExerciseId: number): void {
  db.mutate(() => {
    ...
    db.run('DELETE FROM RoutineSectionExerciseSet WHERE routine_section_exercise_id = ?', [routineExerciseId]);
...
export type PredefinedSetInput = Pick<RoutineSet, 'metricWeight' | 'reps' | 'distanceMetres' | 'durationSeconds' | 'unit'>;
  ...
  db.mutate(() => {
    ...
    sets.forEach((s, i) => {
      db.run(
        ...
        [routineExerciseId, s.metricWeight, Math.round(s.reps), i + 1, s.distanceMetres, Math.round(s.durationSeconds), s.unit],
      );
    });
    db.run('UPDATE RoutineSectionExercise SET populate_sets_type = ? WHERE _id = ?', [populateType, routineExerciseId]);
  });
}
...
export function plannedSetsForSection(db: AppDatabase, sectionId: number, date: string): PlannedSet[] {
  ...
  for (const ex of listSectionExercises(db, sectionId)) {
    ...
    ex.sets.forEach((s, i) => {
      const last = db.get<{ metric_weight: number; reps: number; distance: number; duration_seconds: number; unit: number }>(
      ...
      result.push({
        ...
        durationSeconds: s.durationSeconds || Number(last?.duration_seconds ?? fallback?.durationSeconds ?? 0),
...
export function logRoutineSection(db: AppDatabase, sectionId: number, date: string, sets: PlannedSet[]): void {
  db.mutate(() => {
    for (const s of sets) {
      addSet(db, {
    ...
    for (const g of listRoutineSectionGroups(db, sectionId)) {
      ...
      const exists = db.get('SELECT _id FROM WorkoutGroup WHERE name = ? AND routine_section_id = ? AND date = ?', [
        g.name,
        sectionId,
        date,
      ]);
      if (!exists) createGroup(db, { date, routineSectionId: sectionId, name: g.name, colour: g.colour, exerciseIds: members });
    }
  });
}
