...
export function recalculatePersonalRecords(db: AppDatabase, exerciseId?: number): void {
  db.mutate(() => {
    ...
    for (const id of ids) {
      const type = db.get<{ exercise_type_id: number }>(
        'SELECT exercise_type_id FROM exercise WHERE _id = ?',
        [id],
      );
      if (!type) continue;
      if (type.exercise_type_id !== ExerciseType.WEIGHT_AND_REPS) {
        db.run(
          'UPDATE training_log SET is_personal_record = 0, is_personal_record_first = 0 WHERE exercise_id = ? AND is_personal_record != 0',
          [id],
        );
        continue;
      }
      ...
      for (const s of sets) {
        ...
        if ((s.is_personal_record ? 1 : 0) !== should) {
          db.run(
            'UPDATE training_log SET is_personal_record = ?, is_personal_record_first = ? WHERE _id = ?',
            [should, should, s._id],
          );
        }
      }
