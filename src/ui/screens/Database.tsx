...
export function DatabaseScreen() {
  ...
  return (
    <div className="screen">
      <TopBar back title="Database" subtitle={`FitNotes schema version ${info.version} · ${(info.size / 1024).toFixed(0)} KB`} />
            ...
            {info.tables.map((t) => (
              <div key={t.name} className="stat-row" style={{ display: 'block' }}>
                <div className="row row--between">
                  <span className="mono" style={{ fontSize: 13 }}>{t.name}{!t.known && <span className="chip" style={{ marginLeft: 6 }}>extra</span>}</span>
                  <span className="stat-row__value">{t.rows}</span>
                </div>
                <div className="muted mono" style={{ fontSize: 11, marginTop: 2, wordBreak: 'break-word' }}>{t.columns.join(', ')}</div>
              </div>
            ))}
          </div>
        </div>
