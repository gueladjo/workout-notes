...
export function WorkoutView({
  ...
  if (workout.exercises.length === 0 && !workout.comment) {
    return <div className="muted" style={{ padding: 16 }}>No workout recorded on this date.</div>;
  }
  return (
    <div className="workout-view">
      ...
      {workout.exercises.map((we) => (
        <div key={we.exercise.id} className="workout-view__exercise">
          {we.group && <span className="group-bar" style={{ background: androidColourToHex(we.group.colour) }} />}
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {showCategory && <span className="dot" style={{ background: androidColourToHex(we.exercise.categoryColour) }} />}
              {we.exercise.name}
            </button>
            <ul className="workout-view__sets">
              {we.sets.map((s) => (
                <li key={s.id}>
                  <span>{formatSet(s, we.exercise.typeId, weightUnitFor(we.exercise, settings), settings)}</span>
                  {s.isPersonalRecord && settings.trackPersonalRecords && <Icon name="trophy" size={16} className="trophy" />}
