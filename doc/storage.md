...
- one last time when another instance takes the database over (`AppDatabase.close()`: writes are
  refused from then on, what is unsaved is written, and nothing is written afterwards).
...
| Chosen backup file unreadable    | `pickFile` rejects with `FileReadError` (a cloud file not downloaded, a provider gone); Settings shows the reason in a toast, nothing is touched, Restore stays available                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Two tabs open                    | only one instance runs per origin (`src/app/instance.ts`): a new tab, window or installed app asks the running one over a `BroadcastChannel` to flush and stop, then takes the Web Lock; the stopped one stops its hide/pagehide flushes, closes its database (`AppDatabase.close()`: no more writes, one last save of what is unsaved, nothing written afterwards) and shows "open in another window" with a button to take it back; when that last save failed it says so and offers to save a copy of its data as a backup file. Browsers without Web Locks get no guard (last writer wins) |
| Stored database cannot be opened | `bootstrap()` rejects with `UnreadableDatabaseError` carrying the bytes; the Recovery screen offers to save them as a file, restore a snapshot or start fresh, always keeping the unreadable bytes as a snapshot first (`src/app/recovery.ts`). The takeover handler is registered before the bytes are read, so a window on this screen hands over like a running one: a recovery write in flight finishes first (`holdInstanceLock`), none starts afterwards, and the new window gets the Recovery screen itself                                                                             |
