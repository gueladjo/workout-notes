...
| Two tabs open                    | only one instance runs per origin (`src/app/instance.ts`): a new tab, window or installed app asks the running one over a `BroadcastChannel` to flush and stop, then takes the Web Lock; the stopped one shows "open in another window" with a button to take it back. Browsers without Web Locks get no guard (last writer wins) |
